...
const useActionBar = () => {
  ...
  if (!context) {
    throw new Error(
      "ActionBar components must be used within ActionBarProvider",
    );
  }
  return context;
};
...
const ActionBarProvider = React.forwardRef<
  HTMLDivElement,
...
>(
  (
    ...
    const isOpen =
      mode === "dock" ? true : isControlled ? controlledOpen : internalOpen;
...
const ActionBarHeader = React.forwardRef<
  HTMLDivElement,
...
const ActionBarLogo = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => {
  return (
...
const ActionBarNav = React.forwardRef<
  HTMLElement,
