  ...
  return {
    ...
    frontmatter: {
      ...
      featured: frontmatter.featured
        ? Boolean(frontmatter.featured)
        : undefined,
      component: frontmatter.component
        ? Boolean(frontmatter.component)
        : undefined,
...
function extractCategoryFromMarkdown(
  ...
  const categoryMatch = frontmatterMatch[1].match(
    /category:\s*["']?([^"'\n]+)["']?/,
  );
...
export async function getCraftBySlug(
  ...
  if (!title && content) {
    title =
      extractTitleFromMarkdown(content.markdown, content.frontmatter) ||
      undefined;
  }
  if (!title && component) {
  ...
  if (!category && content) {
    category =
      extractCategoryFromMarkdown(content.markdown, content.frontmatter) ||
      undefined;
  }
