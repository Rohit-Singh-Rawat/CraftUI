...
import {
  ...
  usePanel,
  type CodeFile,
} from "@/components/panels/panel-context";
...
export function CraftLayoutClient({
  ...
  if (previewComponent?.files) {
    const componentPath = componentFileName
      ? `craft/components/${componentFileName}`
      : "";
    const examplePath = exampleFileName
      ? `craft/example/${exampleFileName}`
      : "";
    ...
    for (const file of previewComponent.files) {
      ...
      if (file.content) {
        ...
        const fileName =
          file.path.split("/").pop() || file.path.split("\\").pop() || "file";
