...
export function DependencyCard({
  ...
  return (
    ...
    >
      ...
      {label && (
        <Label className="text-xs px-2 py-0.5 bg-muted rounded font-normal">
          {label}
        </Label>
      )}
    </Link>
...
export function DependenciesContainer({
  children,
  className,
}: DependenciesContainerProps) {
  return (
    <div className={cn("flex flex-wrap gap-3 my-6", className)}>{children}</div>
  );
}
